...
try:
    ...
    print(f"FATAL ERROR: Could not import a required module. Check paths and filenames: {e}")
    sys.exit(1)
...
def step_1_data_receiver(df_client_info: pd.DataFrame, df_transactions: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def run_gasp_pipeline(df_info: pd.DataFrame, df_trans: pd.DataFrame, output_path: str = 'output/gasp_client_report.csv') -> str:
    """
    ...
    try:
        # 1. Initialize Data
        df_info, df_trans = step_1_data_receiver(df_info, df_trans)
