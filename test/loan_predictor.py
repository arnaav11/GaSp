...
import sys
import joblib
import xgboost as xgb # Required to load the XGBoost model correctly
import random

# --- 1. Load the trained model ---

# The model file created by the fixed training script
MODEL_FILE = 'loan_status_model_fixed.joblib'

try:
    # Load the entire pipeline object from the .joblib file
...
# A list of dictionaries representing a mix of positive and negative loan applications.
test_cases = [
    # --- POSITIVE LOAN PROFILES (Expected: Positive) ---
    {
    ...
    # --- NEGATIVE LOAN PROFILES (Expected: Negative) ---
    {
...
# Randomize the order of test cases to ensure no pattern is learned
random.shuffle(test_cases)

# --- 3. Iterate and Predict on the Test Cases ---

print("\n--- Making predictions on randomized test cases ---")

for i, loan_data_dict in enumerate(test_cases, 1):
    try:
        # Separate the expected outcome from the features
        expected_outcome = loan_data_dict.pop('expected')
        
        # Create a DataFrame from the single test case dictionary
        ...
        # Map the prediction from 0/1 to "Negative" or "Positive"
        predicted_outcome = 'Positive' if prediction[0] == 1 else 'Negative'
        
        # Print both the predicted and expected outcomes for comparison
